...
router.post('/Produc',upload.single("articleImage"),async(req,res)=>{
   ...
   try{
     if(!Name||!Prix  || !Catgory ||!req.file.originalname || !Image02 ){
        return res.json({msg:"Entre all input"})
     }
       const ProductAdd = new Product({Name , Prix ,Descraption,Image:req.file.originalname, Catgory , Image02}); 
...
router.post('/productADD02',async(req,res)=>{
   ...
   try{
      ...
      const ProductAdd = new Product({Name ,Descraption , Image02:uploadedResponse.url  ,Prix , Catgory  ,Image:Math.random()  }); 
       await ProductAdd.save();
       return res.json({msg:"Product add"})
